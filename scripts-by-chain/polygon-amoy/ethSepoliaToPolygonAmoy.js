...
const main = async () => {
    ...
    const usdcEthSepoliaContract = new ethers.Contract(ETH_SEPOLIA_USDC_CONTRACT_ADDRESS, usdcAbi, ethSepoliaWallet);
    ...
    const txOptions = {
        gasLimit: ethers.utils.parseUnits('200000', 'wei'), // Set a sufficient gas limit
        gasPrice: ethers.utils.parseUnits('300', 'gwei'), // Set a high gas price (e.g., 200 gwei)
        nonce: 1
    };
    const receiveTx = await polygonAmoyMessageTransmitterContract.receiveMessage(messageBytes[0], attestationSignature, txOptions);
